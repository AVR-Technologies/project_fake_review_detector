		...
		if (isset($_GET["username"])) {
			...
			if (isset($_GET["mobile_no"])) {
				...
				if (isset($_GET["password"])) {
					...
					$query = "SELECT * FROM user WHERE email = '$string_email'";
					...
					if (mysqli_num_rows($query_run)>0) {
						...
						$query = "INSERT INTO user (email, username, mobile, password) VALUES ('$string_email', '$string_username', '$string_mobile_no', '$string_password')";
						$query_run = mysqli_query($connection, $query);
						#insert new admin data throw error if query failed
						if ($query_run) {
							$_SESSION["user_mail"] = $string_email;
